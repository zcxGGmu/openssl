...
/*
 * The FIPS provider also calls OPENSSL_cpuid_setup() during provider init and
 * uses the same RISC-V dispatch helpers as libcrypto. Keep hwprobe available
 * there as well, otherwise provider-local capability state stays empty unless
 * OPENSSL_riscvcap is set explicitly.
 */
